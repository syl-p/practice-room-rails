...
class PracticedActivityTest < ActiveSupport::TestCase
  test "today scope must return today entity" do
    ...
    today_activity =  PracticedActivity.create(user: user, activity: activity, created_at: Date.today)
    yesterday_activity = PracticedActivity.create(user: user, activity: activity, created_at: Date.yesterday)
