module Onboarding::Activities::StepHelper
  ...
  # @param [String] step
  # @return [Boolean]
  def activity_step_nav_enabled?(step)
    @activity.persisted? || step == Onboarding::Activity::STEPS.first
  end

  # @param [String] step
  # @return [String]
  def activity_step_nav_classes(step)
    [
      ...
      ("pointer-events-none opacity-50" unless activity_step_nav_enabled?(step))
    ].compact.join(" ")
  end
end
