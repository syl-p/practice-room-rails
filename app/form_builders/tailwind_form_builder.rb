class TailwindFormBuilder < ActionView::Helpers::FormBuilder
  def text_field(method, options = {})
    form_group(method, options) do
      super(method, merge_default_classes(options, "form-input"))
    end
  end
  ...
  def form_group(method, options, &block)
    ...
    error_span = error_messages.present? ? @template.content_tag(:span, error_messages, class: "text-alert") : "".html_safe

    @template.content_tag(:div, class: "form-group mb-3 #{'has-error' if error_messages.present?}") do
      @template.concat @template.label(@object_name, method, label_text, class: "form-label")
